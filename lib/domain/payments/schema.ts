...
function validateMonthlyPeriod(
  ...
  if (periodEnd <= periodStart) {
    context.addIssue({
      ...
      path: ["period_end"],
      message: PAYMENT_MESSAGES.PERIOD_END_AFTER_START,
    });
    return;
  }

  if (deriveMonthsCovered(input.period_start, input.period_end) > 24) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["period_end"],
      message: PAYMENT_MESSAGES.PERIOD_MAX_24_MONTHS,
    });
  }
}
...
export const registerMonthlyPaymentSchema = monthlyPaymentFieldsSchema
  .extend({ student_discipline_id: z.string().uuid() })
  .strict()
  .superRefine(validateMonthlyPeriod);
...
export const correctMonthlyPaymentSchema = monthlyPaymentFieldsSchema
  .extend({ id: paymentIdSchema })
  .strict()
  .superRefine(validateMonthlyPeriod);

export type CorrectMonthlyPaymentInput = z.infer<typeof correctMonthlyPaymentSchema>;

export const correctClassPaymentSchema = z
  .object({
    id: paymentIdSchema,
    amount: z.number().min(0),
