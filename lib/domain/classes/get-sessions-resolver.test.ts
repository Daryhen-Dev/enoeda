...
describe("getSessionsForRange resolver integration", () => {
  ...
  it("calls $queryRaw and uses resolved teacher instead of default_teacher_id", async () => {
    ...
    mockQueryRaw.mockResolvedValue([{ class_id: CLASS, session_date: "2026-09-08", resolve_effective_teacher: T_B }]);
    setupAuth();
    const r = await getSessionsForRange({ branch_id: BRANCH, start_date: "2026-09-08", end_date: "2026-09-08" });
  ...
  it("falls back to default_teacher_id when resolver returns null", async () => {
    mockFindMany.mockResolvedValue([makeClass(T_A)]);
    mockQueryRaw.mockResolvedValue([{ class_id: CLASS, session_date: "2026-09-08", resolve_effective_teacher: null }]);
    setupAuth();
    const r = await getSessionsForRange({ branch_id: BRANCH, start_date: "2026-09-08", end_date: "2026-09-08" });
    expect(r.success).toBe(true);
  ...
  it("preserves override teacher from class_sessions", async () => {
    ...
    mockFindMany.mockResolvedValue([makeClass(T_A)]);
    mockQueryRaw.mockResolvedValue([{ class_id: CLASS, session_date: "2026-09-08", resolve_effective_teacher: T_OVR }]);
    mockTx.class_sessions.findMany.mockResolvedValue([{ scheduled_class_id: CLASS, session_date: new Date("2026-09-08"), status: "scheduled", suspension_category: null, suspension_reason: null, assigned_teacher_id: T_OVR }]);
    setupAuth();
    const r = await getSessionsForRange({ branch_id: BRANCH, start_date: "2026-09-08", end_date: "2026-09-08" });
    expect(r.success).toBe(true);
