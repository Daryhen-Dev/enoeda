...
import {
  ...
  type MonthlyPaymentActivity,
} from "@/lib/domain/payments/queries"
import {
  formatDate,
...
interface RecentPaymentActivityListProps {
  activity: MonthlyPaymentActivity[]
  branchId: string
}
...
function formatUsd(value: number): string {
  return new Intl.NumberFormat(USER_LOCALE, USD_FORMAT_OPTIONS).format(value)
}

export function RecentPaymentActivityList({
  activity,
  branchId,
}: RecentPaymentActivityListProps) {
  const columns: ColumnDef<TableFeatures, MonthlyPaymentActivity>[] = [
    ...
    },
    {
      ...
      header: PAYMENT_CONSOLE_MESSAGES.DATE,
      cell: ({ row }) => formatDate(new Date(row.original.activity_date)),
    },
    {
      accessorKey: "amount",
      header: PAYMENT_CONSOLE_MESSAGES.AMOUNT,
      cell: ({ row }) => formatUsd(row.original.amount),
    },
    {
      accessorKey: "type",
      header: PAYMENT_CONSOLE_MESSAGES.TYPE,
      cell: ({ row }) =>
        row.original.type === PAYMENT_ACTIVITY_TYPES.MONTHLY
  ...
  return (
    <DataTable
      columns={columns}
      data={activity}
      caption={PAYMENT_CONSOLE_MESSAGES.ACTIVITY_CAPTION}
      emptyState={PAYMENT_CONSOLE_MESSAGES.EMPTY_ACTIVITY}
    />
  )
}
